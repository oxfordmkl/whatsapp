...
class TestStructure:
    ...
    def test_no_schema_or_migration_change(self):
        """H4-d shipped no migration or schema change.
        ...
        unchanged and still enforced: H4-d's OWN committed changeset must
        contain no migrations/ path.
        """
        files = self._phase_commit_files("tests/test_log_rollback_h4d.py")
        if files is None:
            pytest.skip("H4-d is not committed yet")
        migrations = [f for f in files if f.startswith("migrations/")]
        assert migrations == [], (
            f"H4-d committed a migration: {migrations}")
