...
def upgrade():
    ...
    with op.batch_alter_table('billing_invoices') as batch_op:
        batch_op.create_index(
            batch_op.f('ix_billing_invoices_tenant_id'),
            ['tenant_id'],
            unique=False
        )
    ...
    with op.batch_alter_table('tenants') as batch_op:
        ...
        batch_op.add_column(
            sa.Column('billing_subscription_status', sa.String(length=50), nullable=True)
        )
        ...
        batch_op.add_column(
            sa.Column('past_due_at', sa.DateTime(), nullable=True)
        )

        # PostgreSQL-safe boolean default
        batch_op.add_column(
            sa.Column(
                'billing_exempt',
                sa.Boolean(),
        ...
        # Existing rows automatically get USD
        batch_op.add_column(
            sa.Column(
                'currency',
                sa.String(length=3),
        ...
        batch_op.create_unique_constraint(
            'uq_tenants_billing_subscription_id',
